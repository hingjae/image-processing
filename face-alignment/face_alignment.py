...
predictor_file = 'C:/Users/ljh23/anaconda3/Lib/site-packages/face_recognition_models/models/shape_predictor_68_face_landmarks.dat'
image_file = '../image/marathon_02.jpg'
MARGIN_RATIO = 1.5 #(마진 비율)얼굴이 좀 더 크게 나옴
OUTPUT_SIZE = (300, 300)

detector = dlib.get_frontal_face_detector() #얼굴을 찾아줌.
predictor = dlib.shape_predictor(predictor_file) #점을 찍을 좌표를 찾아줌.
...
rects = detector(gray, 1) #얼굴 개수 인식
...
for (i, rect) in enumerate(rects):
    ...
    points = np.matrix([[p.x, p.y] for p in predictor(gray, rect).parts()]) #모든 68개의 점들을 가져옴
    show_parts = points[EYES] #눈에 해당하는 좌표만 찍기
